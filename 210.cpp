...
void Init(){
		...
		switch (map -> objects[i].type){
			case PLAYER:
				...
				if (me[0].radius > map -> objects[i].radius && me[0].skill_level[SHORT_ATTACK]
					|| me[0].skill_level[SHORT_ATTACK]>=5) 
	...
	if (AE_number)//处理AE
	{
		...
		if (AEflag)
		{
			if (me[0].skill_level[SHORT_ATTACK]>=5 && me[0].skill_level[HEALTH_UP]>=4)
				AE_Parameter=0;
			else
				AE_Parameter=100;
	...
	if (me[0].radius>1.08*boss_r  && see_boss)//如果比boss大，去吃
	{
		a2=Minus(boss.pos,me[0].pos);
		//printf("eating boss!\n");
		F=20 * POW(me[0].radius, 2) / POW(length(a2), 3);
		speed=Add(speed,Multiple(F,a2));
	}
...
void Strategy()
{
	...
	/*if (see_boss && dist(me[0].pos, boss.pos) - me[0].radius - boss_r < kShortAttackRange[me[0].skill_level[SHORT_ATTACK]])
		short_attack(me[0]);*///打boss
	upgrade(me[0], SHORT_ATTACK);
	upgrade(me[0], HEALTH_UP);
	if (me[0].skill_level[SHORT_ATTACK] > 2+me[0].skill_level[LONG_ATTACK])
		upgrade(me[0], LONG_ATTACK);
	if (me[0].skill_level[SHORT_ATTACK]==5) 
		upgrade(me[0], SHIELD);
}
